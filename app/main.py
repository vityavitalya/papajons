...
def get_currency(code: str, db: Session = Depends(get_db)):
    ...
    if not currency:
        raise HTTPException(
            ...
            detail=(f"Currency " f"{code} " f"not found"),
        )
...
def create_currency(currency: CurrencyCreate, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            ...
            detail=(f"Currency " f"{currency.code} " f"already exists"),
        )
...
def get_exchange_rate(pair: str, db: Session = Depends(get_db)):
    if len(pair) != 6:
        raise HTTPException(
            ...
            detail=("Pair must contain " "6 characters"),
        )
    ...
    if not rate:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=(f"Rate " f"{base_code} -> " f"{target_code} " f"not found"),
        )
...
def reset_all_rates(
    ...
    """
    Сброс всех курсов обмена.
    Используется для тестирования или массового обновления.
    """
    ...
    return {
        "status": "success",
        "message": f"Удалено курсов: {count}",
        "admin": admin,
    }
...
@app.get(
    ...
    description=("Возвращает метрики: количество валют, " "курсов и размер БД."),
)
...
@app.get(
    "/mobile/rates",
    tags=["Mobile"],
    summary="Список курсов для мобильного приложения",
...
def get_mobile_rates(
    ...
    """
    Эндпоинт для мобильных приложений.
...
def deactivate_currency_endpoint(
    ...
    if crud.deactivate_currency(db, code):
        return {
            "status": "success",
            "message": f"Валюта {code} деактивирована",
        }
...
def activate_currency_endpoint(
    ...
    if crud.activate_currency(db, code):
        return {
            "status": "success",
            "message": f"Валюта {code} активирована",
        }
