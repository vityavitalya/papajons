...
backups/currencies_backup_20240101_120000.db.gz
BACKUP_FILE=$1
...
10.2. Создание systemd-сервиса (для Linux-серверов)
Создаём файл systemd/currency-exchange.service :
fi
# Создаём бэкап текущей БД "на всякий случай"
cp currencies.db "currencies.db.backup_before_restore_$(date
+%Y%m%d_%H%M%S).db"
# Восстанавливаем
