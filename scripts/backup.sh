...
# Рекомендуется запускать по расписанию (cron): 0 2 * * *
/path/to/backup.sh
# Настройки
BACKUP_DIR="./backups"
DB_PATH="./currencies.db"
DATE=$(date +%Y%m%d_%H%M%S)
MAX_BACKUPS=30 # Храним последние 30 резервных копий
...
Создаём файл scripts/restore.sh (восстановление из бэкапа):
# Сжимаем бэкап (опционально, экономит место)
gzip "$BACKUP_DIR/currencies_backup_$DATE.db"
echo " Сжатый размер: $(du -h "$BACKUP_DIR/currencies_backup_$DATE.db.gz"
| cut -f1)"
# Удаляем старые бэкапы (оставляем только MAX_BACKUPS последних)
cd "$BACKUP_DIR"
ls -t currencies_backup_*.db.gz 2>/dev/null | tail -n +$((MAX_BACKUPS +
1)) | xargs -r rm
